...
function getDataJc() {
  ...
  infos?.forEach(el => {
    list.push((el as HTMLElement).innerText)
  })
  return list
}
...
async function getDataRank() {
  ...
  (btn?.children.item(0) as HTMLElement).click();
  await waiting(1)
  ...
  (btn?.children.item(1) as HTMLElement).click();
  await waiting(1)
...
async function getDataZhui(n = 20) {
  ...
  const refreshBtn = section?.querySelector('.video-title .change') as HTMLElement
  let i = 0
  while(i++ < n) {
    const infos = section?.querySelectorAll('.video-info')
    infos?.forEach(el => {
...
async function getDataDuanjv() {
  ...
  while(true) {
    const lastCard = section?.querySelector('.video-card:last-child') as HTMLElement
    if (!lastCard) {
      for(;;) { await waiting(3) }
    }
